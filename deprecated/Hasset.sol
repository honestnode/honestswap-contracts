...
//            )
    ...
    ****************************************/
    ...
    returns (uint256 outputQuantity)
    {
        ...
        honestBasket.prepareSwapBassets(_input, _output, false);
        require(isValid, reason);

        // 3. Deposit the input tokens
        // transfer bAsset to basket
        uint256 quantityTransferred = HassetHelpers.transferTokens(msg.sender, address(honestBasket), _input, inputDetails.bAsset.isTransferFeeCharged, _quantity);
        // Log the pool increase
        honestBasket.increasePoolBalance(inputDetails.index, quantityTransferred);
