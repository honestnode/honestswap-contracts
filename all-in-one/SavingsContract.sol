...
contract SavingsContract is ISavingsContract, Module {
    ...
    // Underlying asset is hUSD
    IERC20 private hUSD;

    // Amount of underlying savings in the contract
    uint256 public totalSavings;
    // Total number of savings weight issued
    uint256 public totalSavingWeight;

    // Amount of weight for each saver
    mapping(address => uint256) public saverWeights;
    bool private automateInterestCollection = true;

    constructor(address _nexus, IERC20 _hUSD)
        public
        Module(_nexus)
    {
        require(address(_hUSD) != address(0), "mAsset address is zero");
        hUSD = _hUSD;
    }
