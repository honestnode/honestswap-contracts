...
contract ChainlinkIntegration is IAssetPriceIntegration, AbstractHonestContract {
    ...
    function initialize(address honestConfiguration, address ethUsdFeeds)
    public initializer() {
    ...
    function setHonestConfiguration(address configuration) external override onlyGovernor {
        require(configuration != address(0), 'ChainlinkIntegration.setHonestConfiguration: configuration must be valid');
        _honestConfiguration = configuration;
    }

    function setEthPriceFeeds(address feeds) external override onlyGovernor {
        require(feeds != address(0), 'ChainlinkIntegration.setEthPriceFeeds: feeds must be valid');
        _ethUsdFeeds = feeds;
    }
