...
contract HonestAssetManager is IHonestAssetManager, WhitelistAdminRole, ReentrancyGuard {
    ...
    function mintTo(address[] memory bAssets, uint[] memory amounts, address recipient) public {
        ...
        for (uint i = 0; i < bAssets.length; ++i) {
            if (amounts[i] > 0) {
                ...
                total = total.add(ERC20(bAssets[i]).standardize(amounts[i]));
            }
        }
    ...
    function redeemManuallyTo(address[] memory bAssets, uint[] memory amounts, address recipient) public {
        ...
        for (uint i = 0; i < bAssets.length; ++i) {
            if (amounts[i] > 0) {
                amounts[i] = ERC20(bAssets[i]).standardize(amounts[i]);
    ...
    function swapTo(address from, address to, uint amount, address recipient) public {
        ...
        uint fee = _chargeFee(ERC20(from).standardize(amount), feeRate);

        address[] memory assets = new address[](1);
        assets[0] = to;
        uint[] memory amounts = new uint[](1);
        amounts[0] = ERC20(from).standardize(amount).sub(fee);
    ...
    external onlyWhitelistAdmin returns (uint[] memory) {
        ...
        (address[] memory assets, uint[] memory amounts) = IHonestVault(_vaultContract).distributeProportionally(to, amount, IHonestVault.Repository.VAULT);
        return amounts;
    }
    ...
    external onlyWhitelistAdmin {
        ...
        for(uint i = 0; i < assets.length; ++i) {
            if (amounts[i] > 0) {
                ...
                total = total.add(ERC20(assets[i]).standardize(amounts[i]));
            }
        }
